...
fn main() {
    ...
    let mut flex = group::Flex::default()
        .with_size(601, 500)
        .column()
        .size_of_parent(); // the outter flex
    ...
    let mut _app_name_label = frame::Frame::default()
        .with_label("launchio\nby philo")
        .set_color(Color::from_rgb(0, 0, 0));
    ...
    input.handle(move |_input, event| {
        // the call back when enter is pressed
...
fn get_executables() -> Vec<String> {
    ...
    if let Some(paths) = env::var_os("PATH") {
        // get the path from the var PATH
